...
export async function getAvailableDates(daysAhead: number = 30) {
  let weeklyAvailability: any[] = [];
  let blackoutDates: any[] = [];
  try {
    weeklyAvailability = await db.availability.findMany();
    blackoutDates = await db.blackoutDate.findMany();
  } catch (e) {
    console.error('Error fetching availability dates:', e);
  }
  ...
  for (let i = 0; i < daysAhead; i++) {
    ...
    // If weeklyAvailability is empty (e.g. initial fallback), allow standard Mon-Sat
    const isDayOpen = dayAvail ? dayAvail.isOpen : dayOfWeek !== 0;

    if (isDayOpen && !blackoutSet.has(dateStr)) {
...
export async function getAvailableTimeSlots(
  ...
  try {
    ...
    const openTime = salonAvail?.openTime || '10:00';
    const closeTime = salonAvail?.closeTime || '20:00';
    const salonOpenMins = timeToMinutes(openTime);
    const salonCloseMins = timeToMinutes(closeTime);
    ...
    console.error('getAvailableTimeSlots DB error:', error);
    return [];
  }
}
